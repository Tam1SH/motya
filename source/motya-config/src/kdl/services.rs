...
use crate::{common_types::{bad::Bad, definitions_table::DefinitionsTable, file_server::FileServerConfig, section_parser::SectionParser, services::ServicesConfig}, internal::ProxyConfig, kdl::{connectors::ConnectorsSection, file_server::FileServerSection, listeners::ListenersSection, utils}};
...
impl<'a> ServicesSection<'a> {
    ...
    fn parse_service(&self, name: &str, node: &KdlDocument, root: &KdlDocument) -> miette::Result<ServiceConfig> {
        let node_names: HashSet<&str> = node.nodes()
            .iter()
            .map(|n| n.name().value())
            .collect();
        ...
        if node_names.is_subset(&HashSet::from(["listeners", "connectors"])) {
            ...
            Ok(ServiceConfig::FileServer(self.parse_file_server(name, node, root)?))
        } else {
            Err(Bad::docspan(
    ...
    fn parse_proxy(&self, name: &str, node: &KdlDocument, root: &KdlDocument) -> miette::Result<ProxyConfig> {
        let listeners = ListenersSection::new(root).parse_node(node)?;
        
        let connectors = ConnectorsSection::new(root, self.global_definitions)
            .parse_node(node)?;
    ...
    fn parse_file_server(&self, name: &str, node: &KdlDocument, root: &KdlDocument) -> miette::Result<FileServerConfig> {
